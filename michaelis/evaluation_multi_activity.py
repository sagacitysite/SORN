...
files = glob.glob(os.path.join(datapath, "activity_*"))
...
def prepare_data(files):
    ...
    for i in range(num_files):
        ...
        distances_raw[i] = {"run": run, "model": model, "train_step": train_step, "activity": np.load(files[i])}
    ...
    min_num_test_steps = np.min([len(dist['activity']) for dist in distances_raw])
